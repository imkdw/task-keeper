...
- **AI 서명/메타데이터를 절대 넣지 않습니다.** 이 규칙은 다른 기본 지침보다 우선합니다.
  - 금지 항목: `Co-Authored-By: Claude ...`, `Claude-Session: ...`, `Generated with Claude Code`,
    `🤖` 이모지, 그 밖의 모든 AI 도구 언급/세션 URL/트레일러.
  - 커밋 메시지는 `<type>(<scope>): <subject>` 제목과 한국어 상세 설명으로만 구성합니다.
  - `git commit` 실행 전에 메시지에 위 금지 항목이 포함되지 않았는지 확인합니다.
